...
// datos de cheques
$cheq_list = json_decode($_POST['datos']);
// datos para el canje
...
///////////////////////////////////////
$cant = count($cheq_list)/5; // cantidad de datos a cargar
...
if($op > 0)
{
	...
	while($c <= $cant)
	{
		...
		$n_cheque = $cheq_list[$j];
		$banco   = $cheq_list[$j+1];
		$emisor = $cheq_list[$j+2];
		$vencimiento = $cheq_list[$j+3];
		$importe = $cheq_list[$j+4];
			
		$insert_cheq = "INSERT IGNORE INTO cheques_cartera VALUES 
		('',
		'$n_cheque',
		'$fecha',
		'$banco',
		...
		$res = mysqli_query($connection, $insert_cheq);
	...
	$insert_mc = "INSERT IGNORE INTO caja_gral VALUES 
			('','$numero_caja','$fecha','$fecha','$det1',0,'$cantidad',0,0,0,0,'$cheque_a_sumar',4)";
	//$result_inert_mc = mysqli_query($connection, $insert_mc);
	mysqli_query($connection, $insert_mc);

	$insert_mc = "INSERT IGNORE INTO caja_gral VALUES 
			('','$numero_caja','$fecha','$fecha','$det2',0,0,'$cantidad','$pesos_a_restar',0,0,0,1)";
	//$result_inert_mc = mysqli_query($connection, $insert_mc);
	mysqli_query($connection, $insert_mc);																	   							
